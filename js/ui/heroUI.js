...
function setHeroMenuExpanded(expanded) {
    const heroMenuCategory =
        document.querySelector(
            "[data-hero-menu-category]"
        );
    ...
    const heroToggle =
        heroMenuCategory
            ? heroMenuCategory.querySelector(
                ".hero-menu-toggle"
            )
            : null;

    if (!heroSubitems || !heroToggle) {
        return;
    }

    heroSubitems.hidden = !expanded;

    heroMenuCategory.classList.toggle(
        "is-open",
        expanded
    );
    ...
    localStorage.setItem(
        "idler_hero_menu_expanded",
        expanded ? "true" : "false"
    );
}

function toggleHeroMenuSubitems() {
    const heroSubitems =
        document.getElementById(
            "hero-menu-subitems"
        );
...
function restoreHeroMenuSubitems() {
    const savedState =
        localStorage.getItem(
            "idler_hero_menu_expanded"
        );
    ...
    if (
        ...
        document.addEventListener(
            ...
            () => {
                ...
                if (
                    typeof restoreHeroMenuSubitems ===
                    "function"
                ) {
                    restoreHeroMenuSubitems();
                }
            },
            {
                once: true
